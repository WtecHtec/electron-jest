...
export default memo((porps) => {
  ...
  const onDeleteNode = () => {
    ...
    if (['logic_close','logic_back','logic_reload','logic_pdf', 'logic_func', 'logic_new_page', 'logic_js_func', 'logic_intercepting_response', 'logic_fetch_request'].includes(node.type)) return <LogicBaseDrawer node={node}></LogicBaseDrawer>
    return <></>
	}
	return (
		<>
