...
const OptExistsItem  = forwardRef((props, ref) => {
  ...
  const getPickDataByLoop = (parentXpath, fixXpath, count, pickType) => {
    ...
    <Divider dashed></Divider>
		<Space.Compact block style={{ alignItems: 'center' }}>
			<Tooltip placement="top" title={<span>检查当前元素\检查列表下遍历元素,配合循环使用</span>}>
				<span style={{ flexShrink: 0 }}>检查方式：</span>
			</Tooltip>
		...
		</Space.Compact>
    {
      pickData.pickMethod === 'list' && <>
        <Divider dashed></Divider>
        <Space.Compact block style={{ alignItems: 'center' }}>
          <Tooltip placement="top" title={<span>判断元素内容,如:text判断容器纯文本,[只是方便查看效果,实际还是检查元素是否存在]</span>}>
            <span style={{ flexShrink: 0 }}>判断内容：</span>
          </Tooltip>
